...
public class FixedLengthFileReaderTest
{
    static FixedLengthFileReaderTest()
    {
    ...
    public void ReadLine(string encodingName, string newline, int? bufferSize)
    {
        ...
        using var reader = new Claude.FixedLengthFileReader(stream, encoding, newline, bufferSize: bufferSize ?? 4096);

        // Act
        reader.Read().Should().BeTrue();
        reader.CurrentLine.Should().BeEquivalentTo(encoding.GetBytes(first));
        reader.Read().Should().BeTrue();
        reader.CurrentLine.Should().BeEquivalentTo(encoding.GetBytes(second));
        reader.Read().Should().BeTrue();
        reader.CurrentLine.Should().BeEquivalentTo(encoding.GetBytes(third));
        reader.Read().Should().BeTrue();
        reader.CurrentLine.Should().BeEquivalentTo(encoding.GetBytes(forth));
