...
public class ByteStreamReader : IDisposable, IAsyncDisposable
{
    ...
    public byte[]? ReadLine()
    {
        ...
        do
        {
            ...
            if (0 <= indexOfNewline)
            {
                ...
                {
                    retVal = Concat(_bytes.Slice(0, _pos), bufferSpan.Slice(0, indexOfNewline));
    ...
    private Span<byte> Grow(scoped Span<byte> _bytes, int additionalCapacityBeyondPos)
    {
        ...
        _valueByteArrayBuilderBuffer = ArrayPool<byte>.Shared.Rent(newCapacity);

        _bytes.Slice(0, _pos).CopyTo(_valueByteArrayBuilderBuffer);

        return _valueByteArrayBuilderBuffer;
    }
