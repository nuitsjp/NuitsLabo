    ...
    public byte[]? ReadLine()
    {
        ...
        do
        {
            ...
            if (0 <= indexOfNewline)
            {
                ...
                {
                    retVal = Concat(((Span<byte>)_valueByteArrayBuilderBuffer).Slice(0, _pos), bufferSpan.Slice(0, indexOfNewline));
            ...
            _pos = Append(bufferSpan, _pos);
        } while (0 < ReadByteBuffer());

        return ((Span<byte>)_valueByteArrayBuilderBuffer).Slice(0, _pos).ToArray();
    ...
    public int Append(scoped ReadOnlySpan<byte> value, int _pos)
    {
        ...
        if (_pos > span.Length - value.Length)
        {
            ...
            var newCapacity = (int)Math.Max(
                (uint)(_pos + value.Length),
                Math.Min((uint)span.Length * 2, arrayMaxLength));

            // Make sure to let Rent throw an exception if the caller has a bug and the desired capacity is negative.
            // This could also go negative if the actual required length wraps around.
            _valueByteArrayBuilderBuffer = new byte[newCapacity];

            span.Slice(0, _pos).CopyTo(_valueByteArrayBuilderBuffer);
        }

        value.CopyTo(_valueByteArrayBuilderBuffer.AsSpan().Slice(_pos));
