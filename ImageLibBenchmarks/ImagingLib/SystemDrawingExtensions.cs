...
public static class SystemDrawingExtensions
{
    ...
    public static unsafe Binary ToBinary(this byte[] data)
    {
        ...
        try
        {
            ...
            // グレースケール画像のメモリ確保
            var grayIntPtr = Marshal.AllocCoTaskMem(srcStride * srcBitmapData.Height);
            try
            {
                ...
                for (var y = 0; y < srcBitmapData.Height; y++)
                {
                    for (var x = 0; x < srcBitmapData.Width; x++)
                    {
                        ...
                        grayScale[srcStride * y + x] = (byte)((r * RedFactor + g * GreenFactor + b * BlueFactor) >> 10);
                    }
                }
                ...
                // 2回目のループ：二値化処理（元画像から再計算せず、grayScaleを利用）
                for (var y = 0; y < height; y++)
                {
                    for (var x = 0; x < width; x++)
                    {
                        var grayValue = grayScale[y * srcStride + x];
                        if (grayScaleThreshold <= grayValue)
                        {
                            // 書き込み先メモリの対応バイトにビットをセット
